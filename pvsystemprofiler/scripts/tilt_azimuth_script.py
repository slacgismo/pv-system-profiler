...
def run_failsafe_ta_estimation(dh, nrandom, threshold, lon_input=None, lat_input=None, tilt_input=None,
                               azimuth_input=None, lat_true_value=None, tilt_true_value=None,
                               azim_true_value=None, gmt_offset=None, cp=None, tq=None):

    try:
        runs_ta_estimation = True
        ta_study = TiltAzimuthStudy(data_handler=dh, nrandom_init_values=nrandom, daytime_threshold=threshold,
                                    lon_input=lon_input, lat_input=lat_input, tilt_input=tilt_input,
                                    azimuth_input=azimuth_input, lat_true_value=lat_true_value,
                                    tilt_true_value=tilt_true_value, azimuth_true_value=azim_true_value,
        ...
        if lat_input is not None:
            cols.append('latitude')
        if tilt_input is not None:
            cols.append('tilt')
        if azimuth_input is not None:
            cols.append('azimuth')
        if lat_input is not None:
        ...
        if col_label.find(inputs_dict['power_column_label']) != -1:
            ...
            if inputs_dict['system_summary_file'] is None or system_id in metadata_dict.keys():
                ...
                if inputs_dict['gmt_offset']:
                    ...
                    gmt_offset = metadata_dict[system_id][2]
                real_longitude = metadata_dict[system_id][3]  # not used in current implementation
                real_latitude = metadata_dict[system_id][4]
                real_tilt = metadata_dict[system_id][5]
                real_azimuth = metadata_dict[system_id][6]
                longitude_input = metadata_dict[system_id][7]
                latitude_input = metadata_dict[system_id][8]
                tilt_input = metadata_dict[system_id][9]
                azimuth_input = metadata_dict[system_id][10]
                ...
                if passes_pipeline:
                    results_df, passes_estimation = run_failsafe_ta_estimation(dh, nrandom=1, threshold=None,
                                                                               lon_input=longitude_input,
                                                                               lat_input=latitude_input,
                                                                               tilt_input=tilt_input,
                                                                               azimuth_input=azimuth_input,
                                                                               lat_true_value=real_latitude,
                                                                               tilt_true_value=real_tilt,
                                                                               azim_true_value=real_azimuth,
