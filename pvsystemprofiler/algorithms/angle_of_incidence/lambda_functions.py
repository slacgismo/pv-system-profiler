...
declination (delta), the hour angle (omega) , latitudeitude (phi), tilt (beta) and azimuth (gamma). The declination and the
hour angle are treated as input parameters for all cases. latitudeitude, tilt and azimuth can be given as input parameters
(precalculatitudees) or left as unknowns. In total, seven different combinations arise from having these three parameters
...
the 'bounds' tuple containing the bounds for the variables. Bounds for latitudeitude are -90 to 90. Bounds for tilt are 0 to
...
def select_function(latitude=None, tilt=None, azimuth=None):
    """
    :param latitude: (optional) latitude input valie in Degrees.
