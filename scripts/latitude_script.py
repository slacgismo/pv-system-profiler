...
df_site = pd.read_csv(sites_file, index_col=0)
        ...
        for sys_ix, sys_id in enumerate(site_system_dict[site_id]):
            if sys_id not in checked_systems:
                ...
                if data_available:
                    ...
                    lat_study.run()
                    partial_df = lat_study.results.copy()
                    ...
                    partial_df = partial_df.reindex(columns=['site', 'system', 'length', 'data sampling',
                                                             'data quality score', 'data clearness score',
                                                             'inverter clipping', 'declination_method',
                                                             'daylight_calculation', 'data_matrix', 'threshold',
