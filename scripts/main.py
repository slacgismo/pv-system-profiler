...
if __name__ == '__main__':
    ...
    script_location = str(sys.argv[7])
    output_folder_location = str(sys.argv[8])
    main_class = get_config(ifl=input_file_location, ofl=output_folder_location, skf=ssh_key_file, au=aws_username,
                            ain=aws_instance_name, ar=aws_region, ac=aws_client)
    ...
    while jj < ll:
        ...
        part = get_config(part_id=i, ix_0=ii, ix_n=jj, n_part=n_part, ifl=input_file_location,
                          ...
                          ar=aws_region, ac=aws_client, script_location=script_location)
